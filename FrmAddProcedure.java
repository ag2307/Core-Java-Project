...
class FrmAddProcedure extends JInternalFrame
{
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	JLabel lblDate,lblPatient_id,lblIPD_no,lblRoom_no,lblPCategory,lblMedicine,lblAmount;
	JTextField txtDate,txtPatient_id,txtRoom_no,txtAmount;
	TextField txtID;
	...
	{	
		...
		lblPCategory=new JLabel("procedure category");
		lblMedicine=new JLabel("procedure");
		...
		try
		{
			...
			rst=dc.executeQuery("select * from pprocedure where pcat_id="+cat);
			while(rst.next())
			{
		...
		jcbmc.addItemListener(new ItemListener()
		{
		 	public void itemStateChanged(ItemEvent ie)
		 	{
				...
				int id=0;
		 		try
				{
					rst=dc.executeQuery("select pcat_id from pcat where pcat_name='"+(String)jcbmc.getSelectedItem()+"'");
					rst.next();
					id=rst.getInt(1);
					rst=dc.executeQuery("select * from pprocedure where pcat_id="+id);
					while(rst.next())
					{
						jcbmd.addItem(rst.getString(2));
					}
		...
		jcbmd.addFocusListener (new FocusListener () 
		{
   			 public void focusLost (FocusEvent e) 
   			 {
   			 	try
				{
					rst=dc.executeQuery("select price from pprocedure where p_name='"+(String)jcbmd.getSelectedItem()+"'");
					if(rst.next()==true)
					{
						txtAmount.setText(rst.getString(1));
					}
				}
		...
		btnSave.addActionListener(new ActionListener()
		{
		 	public void actionPerformed(ActionEvent ae)
		 	{
		 		...
		 		ResultSet rst2=dc.executeQuery("select * from pprocedure where p_name='"+(String)jcbmd.getSelectedItem()+"'");
		 		try{
		 		rst2.next();
		 		s3=rst2.getString("pcat_id");
		 		s4=rst2.getString("p_id");
		 		}
		 		catch(SQLException e){
		 		}
		 		int s5=Integer.parseInt(txtAmount.getText());
		 	if(s4.isEmpty())
		        {
		        	System.out.println("hoola hoola hoop");
                 JOptionPane.showMessageDialog(null,"procedure can't be blank","Error",JOptionPane.ERROR_MESSAGE);
			     ...
			     return;
		 			...
		 			dc.executeOther("Insert into ipdprocedure values('"+s1+"',"+s[0]+",'"+s3+"','"+s4+"',"+s5+")");
		 	dispose();
		 	//	null.reload();
		 		//null.jsp.repaint();
		 		//null.repaint();*/
		...
		p1.add(lblPCategory);p1.add(jcbmc);
